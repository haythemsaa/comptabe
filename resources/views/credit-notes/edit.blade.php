<x-app-layout>
    <x-slot name="title">Modifier {{ $creditNote->credit_note_number }}</x-slot>

    <script>
        window.creditNoteForm = function() {
            return {
                partnerId: '{{ $creditNote->partner_id }}',
                lines: @json($creditNote->lines->map(fn($l, $i) => [
                    'id' => $i + 1,
                ...
                lineIdCounter: {{ $creditNote->lines->count() }},
    ...
    @section('breadcrumb')
        ...
        <span class="text-secondary-900 dark:text-white font-medium">Modifier {{ $creditNote->credit_note_number }}</span>
    @endsection

    <form
        method="POST"
        action="{{ route('credit-notes.update', $creditNote) }}"
    ...
    >
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-secondary-900 dark:text-white">Modifier {{ $creditNote->credit_note_number }}</h1>
                <p class="text-secondary-600 dark:text-secondary-400">Modifier les détails de cette note de crédit</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('credit-notes.show', $creditNote) }}" class="btn btn-secondary">Annuler</a>
                    ...
                    <div class="card-body space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="md:col-span-2">
                                ...
                                >
                                    ...
                                    @foreach($partners as $partner)
                                        <option value="{{ $partner->id }}" {{ $creditNote->partner_id == $partner->id ? 'selected' : '' }}>
                            ...
                            <div class="md:col-span-2">
                                ...
                                >{{ old('reason', $creditNote->reason) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
