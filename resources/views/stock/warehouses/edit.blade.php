    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1">
                ...
                <span class="text-muted">/</span>
                Modifier {{ $warehouse->name }}
            </h4>
        </div>
        <a href="{{ route('stock.warehouses.show', $warehouse) }}" class="btn btn-outline-secondary">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between mb-2">
                            ...
                            <span class="fw-medium">{{ $warehouse->productStocks()->count() }}</span>
                        </div>
                        <div class="d-flex justify-content-between mb-2">
                            <span class="text-muted">Valeur totale</span>
                            <span class="fw-medium">{{ number_format($warehouse->productStocks()->sum(\DB::raw('quantity * unit_cost')), 2, ',', ' ') }} €</span>
                        </div>
                        <div class="d-flex justify-content-between">
                            <span class="text-muted">Créé le</span>
                            <span class="fw-medium">{{ $warehouse->created_at->format('d/m/Y') }}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="allow_negative_stock" name="allow_negative_stock" value="1"
                                   {{ old('allow_negative_stock', $warehouse->allow_negative_stock) ? 'checked' : '' }}>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-grid gap-2">
                            ...
                            @if(!$warehouse->is_default && $warehouse->productStocks()->count() == 0)
                            <form action="{{ route('stock.warehouses.destroy', $warehouse) }}" method="POST"
                                  onsubmit="return confirm('Supprimer cet entrepôt ? Cette action est irréversible.')">
                                @csrf
                                @method('DELETE')
