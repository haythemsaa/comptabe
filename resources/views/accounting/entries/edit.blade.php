<x-app-layout>
    <x-slot name="title">Modifier l'écriture {{ $entry->entry_number }}</x-slot>
    ...
    @section('breadcrumb')
        ...
        <span class="text-secondary-900 dark:text-white font-medium">Modifier {{ $entry->entry_number }}</span>
    @endsection

    <form
        method="POST"
        action="{{ route('accounting.entries.update', $entry) }}"
        x-data="journalEntry(@js($entry->lines->map(fn($l) => [
            'id' => $l->id,
    ...
    >
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-secondary-900 dark:text-white">Modifier l'écriture</h1>
                <p class="text-secondary-600 dark:text-secondary-400">{{ $entry->entry_number }}</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('accounting.entries.show', $entry) }}" class="btn btn-secondary">Annuler</a>
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    @foreach($journals as $journal)
                                        <option value="{{ $journal->id }}" {{ old('journal_id', $entry->journal_id) == $journal->id ? 'selected' : '' }}>
                        ...
                        <div class="space-y-3 mt-3">
                            <template x-for="(line, index) in lines" :key="line.id">
                                <div class="grid grid-cols-12 gap-4 items-start p-3 bg-secondary-50 dark:bg-secondary-800/50 rounded-xl">
                                    ...
                                    <div class="col-span-2 md:hidden flex justify-end">
                                        <button
                                            type="button"
    ...
    @push('scripts')
    <script>
        function journalEntry(existingLines = []) {
            return {
                lines: existingLines.length > 0 ? existingLines.map((l, i) => ({...l, id: l.id || i + 1})) : [
                    { id: 1, account_id: '', description: '', debit: 0, credit: 0 },
                    { id: 2, account_id: '', description: '', debit: 0, credit: 0 },
                ],
                nextId: existingLines.length > 0 ? Math.max(...existingLines.map(l => l.id || 0)) + 1 : 3,
